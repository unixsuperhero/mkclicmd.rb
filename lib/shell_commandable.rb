...
class Josh
  attr_accessor :vals
  attr_accessor :argmgr, :cargv

  def initialize(argmgr=nil, cargv=nil, **vals)
    @vals = vals
    @argmgr = argmgr
    @cargv = cargv
  end
...
class ArgumentHelper
  class << self
    ...
    def init(list=ARGV.clone)
      ...
      j = Josh.new(list, @original_args, actual_argv: ARGV, argf: ARGF)
      binding.pry
...
module ShellCommandable
  ...
  def self.included(base)
    ...
    base.subcommand_file ||= ShellCommandable.loading_file
  ...
  module ClassMethods
    ...
    def load_subcommands_by_prefix(prefix)
      ...
      Dir[subcmd_filename_pattern].each do |subcmd|
        ShellCommandable.loading_files.push subcmd
        load subcmd
      end
    end
...
class HeroHelper
  class << self
    def glob_path_env(glob)
      ENV['PATH'].split(?:).flat_map{|path|
        Dir.glob(File.join(path.sub(/\/*$/,''),glob))
      }
    end
    ...
    def mkdir_p(path)
      ...
      dirs.each do |dir|
        puts format('Making directory: "%s"', dir)
        Dir.mkdir(dir) unless Dir.exist?(dir)
      end
    end

    def file_mkdir_p(file)
      path = File.dirname(file)
      mkdir_p(path)
    end

    def file_append(file, &block)
